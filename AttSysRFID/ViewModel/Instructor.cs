...
namespace AttSysRFID.ViewModel
{
    public class Instructor:IDisposable
    {
        ...
        public bool Compare(T_InstructorInformation value)
        {
            using (AttMonSysRFIDDataContext dc = new AttMonSysRFIDDataContext())
            {
                return dc.T_InstructorInformations.Where(x => x.Address == value.Address && x.BDay == value.BDay && x.ContactNo == value.ContactNo && x.EmployeeID == value.EmployeeID && x.FirstName == value.FirstName && x.Gender == value.Gender && x.LastName == value.LastName).FirstOrDefault() == null ? true : false;
